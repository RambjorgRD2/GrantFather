...
-- Fixed version with proper UUID handling
...
BEGIN
    ...
    -- Check if test organizations already exist to avoid duplicates
    IF EXISTS (SELECT 1 FROM public.organizations WHERE name = 'Test Organization Alpha' AND created_by = target_user_id) THEN
        RAISE NOTICE 'Test organizations already exist for this user, skipping creation';
        RETURN;
    END IF;
    
    -- Create test organizations with pre-generated UUIDs
